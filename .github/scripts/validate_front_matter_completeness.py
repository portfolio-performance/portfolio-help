...
# Required front matter fields (case-insensitive)
...
def validate_front_matter(md_files):
    """Validate the front matter of markdown files."""
    issues = []
...
def print_summary(stats):
    ...
    if total_files > 0:
        ...
        print("📈 Overall front matter completeness rate:")
        print(
...
if __name__ == "__main__":
    print(
        "🔍 Scanning markdown files for missing required front matter fields in: "
    ...
    if issues:
        print("\n❌ Missing front matter fields detected:")
        for lang, file_path, description in issues:
            print(f"  [{lang}] File '{file_path}' ({description})")
        print(
            f"\n❌ Validation failed: {len(issues)} file(s) with missing front matter fields.\n"
        )
        sys.exit(1)
    else:
        print("\n✅ Validation successful: all required front matter fields present.\n")
        sys.exit(0)
